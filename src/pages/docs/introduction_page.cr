...
[Crystal lang](https://crystal-lang.org/), 由 [Manas](https://manas.tech/) 的
...
如果结合 Ruby 和静态强类型语言的有点，性能不可以更好一些吗？

Crystal 当前主要维护者是 manas 的 [Johannes Müller @straight-shoota](https://github.com/straight-shoota)
以及社区贡献。

早期，Crystal 的宣传卖点主要是：fast as C, Slick as Ruby (速度和 C 语言一样快，代码和 Ruby 一样漂亮)
...
Ruby 社区的开发者加入，部分核心开发者认为学习 Crystal 是不必学习 Ruby 的，这点我**不**认同。
至少到目前 2024 年截至，大量的 Ruby 优秀图书仍旧是学习 Crystal 的最好材料。

事实上，[最早版本的 Crystal](https://github.com/asterite/crystal) 的确是使用 Ruby 写的，
直到 14 个月之后的 `2013年11月14日` 才实现自举（即：使用 Crystal 编写 Crystal）
...
但是，笔者（一个十年的老 Rubyist）从个人角度，认为官方忽略了 Crystal 语言最重要一个特性，

## 空安全（Null Safety）
HEREDOC

    missing_info <<-'HEREDOC'
...
这一行为导致了不计数的错误、漏洞和系统崩溃，很可能在过去四十年造成数十亿美元的痛苦和损失！
                 ...
                 1980 年图灵奖获得者，快速排序发明人 托尼·霍尔 （Tony Hoare）
HEREDOC

    markdown <<-'HEREDOC'
Crystal 使用称为 ``union type`` 的类型系统，从一开始就设计为尽可能的类型安全。

类似于 Rust, 编译器会在 `编译时` 最大限度捕获有关类型安全的错误，当编译通过时，
用户可以避免 90% 的运行空指针或类型错误。
...
代码看起来和 Ruby 神似的同时，不但拥有极好的性能，同时拥有强大的编译时类型安全检查，
带来的一个额外好处是，相较于 Ruby 非常少的测试，更大胆的重构，笔者特别享受这种根据
编译时错误驱动的开发模式，只要编译器通过通过之后，你已经解决了绝大多数有关类型的错误，
这当然也包括 Ruby 里面造成上面 “十亿美金错误” 的空引用(nil)错误。
...
golang 首先使用 goroutine/Fiber 基于 Communicating Sequential Processes [CSP](http://www.usingcsp.com/cspbook.pdf) 的并发模型实现，

Crystal 也从语言内部从一开始就支持了基于 Fiber/Chanel 模型是实现了 CSP 模型。

这里强调一下 Crystal 语言在国内社区（例如某乎）上最被误解的一点，那就是 “不支持多线程”，
这显然是不对的，Crystal __从一开始就支持和 Ruby 一样的多线程使用方式__，我们这里讨论的是，
类似于 golang 的 [M:N concurrency](https://pauldigian.com/advanced-go-goroutines-the-basics#mn-concurrency),
即：在打开多线程的情况下, M 个 Fiber 自动运行在 N 个操作系统线程中。
