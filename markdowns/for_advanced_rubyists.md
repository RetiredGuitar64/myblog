...
即：`@@类变量` 是属于类自身的变量，每一个子类也会继承父类的类变量值，但是会创建自己**独立的**类变量实例, 因此修改子类的类变量的值，不会影响父类，反之亦然。

     their type is the same, but each class has a different runtime value.
...
因为当子数组作为参数传递给代码块时，会自动代码块参数形参的个数，auto expanding，
...
而 Crystal 中 &. 用法含义完全不同，并且更强大，&. 中的 & 代表传递到代码块的第一个参数。
...
(1..10).map(&:size), 但是更强大，正如上面的例子，它还允许接受参数，甚至链式调用：
